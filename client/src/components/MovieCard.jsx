...
const MovieCard = ({movie}) => {
  ...
  return (
    ...
    hover:-translate-y-1 transition duration-300 w-66'>
        ...
        src={movie.backdrop_path} alt="" className='rounded-lg h-52 w-fullobject-cover object-right-bottom cursor-pointer'/>
        <p className='font-semibold mt-2 truncate'>{movie.title}</p>

        <p className='text-sm text-gray-400 mt-2'>
            {new Date(movie.release_date).getFullYear()} . {movie.genres.slice(0,3).map
